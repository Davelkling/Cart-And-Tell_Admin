                ...
                <div class="bg-white rounded-3xl w-full p-4 h-full mt-4">
                    ...
                    <div v-if="createModal" class="absolute h-full w-full bg-transparent flex left-0 p-6 justify-center items-center">
                        <div class="w-[500px] bg-white rounded-lg shadow-md p-4 z-4 flex flex-col" @click="">
                            ...
                            <div class="mt-4">
                                <label class="block mb-1 font-bold text-lg" for="name">MerchantId</label>
                                <input v-model="formData.merchantId" id="name" type="number" name="name" class="py-2 px-3 border border-gray-300 focus:border-blue-300 focus:outline-none focus:ring focus:ring-blue-200 focus:ring-opacity-50 rounded-md shadow-sm disabled:bg-gray-100 mt-1 block w-full" />
                            </div>
                            <div class="mt-4">
                            ...
                            <div class="mt-4">
                                ...
                                <textarea v-model="formData.description" class="py-2 px-3 border border-gray-300 focus:border-blue-300 focus:outline-none focus:ring focus:ring-blue-200 focus:ring-opacity-50 rounded-md shadow-sm disabled:bg-gray-100 mt-1 block w-full"></textarea>
                            </div>
                            <div class="mt-4">
                                <label class="block mb-1 font-bold text-lg" for="name">Amount</label>
                                <input v-model="formData.amount" id="name" type="number" name="name" class="py-2 px-3 border border-gray-300 focus:border-blue-300 focus:outline-none focus:ring focus:ring-blue-200 focus:ring-opacity-50 rounded-md shadow-sm disabled:bg-gray-100 mt-1 block w-full" />
                            </div>
                            ...
                            <div class="flex-1 basis-0 flex justify-center items-end mt-4">  
                                <button @click="create" value="create" type="button" class="w-full inline-flex items-center justify-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold capitalize text-white hover:bg-blue-700 active:bg-blue-700 focus:outline-none focus:border-blue-700 focus:ring focus:ring-blue-200 disabled:opacity-25 transition">Create product</button>
                            </div>
                    ...
                    <div v-if="editModal" class="absolute h-full w-full bg-transparent flex left-0 p-6 justify-center items-center">
                        <div class=" w-[500px] bg-white rounded-lg shadow-md p-4 z-4 flex flex-col" @click="">
                            ...
                            <div class="flex-1 basis-0 flex justify-center items-end mt-4">  
                                <button @click="update" value="create" type="button" class="w-full inline-flex items-center justify-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold capitalize text-white hover:bg-blue-700 active:bg-blue-700 focus:outline-none focus:border-blue-700 focus:ring focus:ring-blue-200 disabled:opacity-25 transition">Update product</button>
                            </div>
                            <p v-if="isLoading" class="text-center">Loading</p>
                        </div> 
                    </div>
                    ...
                    <div v-if="deleteModal" class="absolute h-full w-full bg-transparent flex left-0 p-6 justify-center items-center">
                        <div class="h-[500px] w-[500px] bg-white rounded-lg shadow-md p-4 z-4 flex flex-col" @click="">
                            ...
                            <div class="mt-4">
                                <label class="block mb-1 font-bold text-lg" for="name">Name</label>
                                <p>{{products!.filter(product => product.id == selectedId)[0].name }}</p>
                            </div>
                            <div class="mt-4">
                                <label class="block mb-1 font-bold text-lg" for="name">Image</label>
                                <div class="flex justify-center items-center">
                                    <img  class="h-[200px] object-cover" :src="products!.filter(product => product.id == selectedId)[0].image" alt="">
                                </div>
                            </div>
                            <div class="flex-1 basis-0 flex justify-center items-end">  
                                <button @click="Delete" value="create" type="button" class="w-full inline-flex items-center justify-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold capitalize text-white hover:bg-blue-700 active:bg-blue-700 focus:outline-none focus:border-blue-700 focus:ring focus:ring-blue-200 disabled:opacity-25 transition">Delete product</button>
                            </div>
                        </div> 
                    </div>
                    <div class="flex justify-between items-center">
                        <div class="flex justify-start items-center">
                            ...
                            <p v-else class="font-bold">{{ (products!.length) ? products!.length : 0 }}</p>
                        </div>
                        <div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">merchantId</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200" >
                            ...
                            <tr v-else v-for="product in products" :key="product.id">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <p class="w-[250px] overflow-hidden whitespace-nowrap text-ellipsis">{{ product.name }}</p>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <p class="w-[250px] overflow-hidden whitespace-nowrap text-ellipsis">{{ product.description }}</p>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <p>{{ product.amount}}</p>
                                </td> 
                                <td class="px-6 py-4 whitespace-nowrap  text-sm font-medium">
                                    <button @click="openModal" value="edit" :merchantId="product.merchantId.toString()" :id="product.id.toString()" type="button" class="text-indigo-600 hover:text-indigo-900">Edit</button>
                                    <button @click="openModal" value="delete" :merchantId="product.merchantId.toString()" :id="product.id.toString()" type="button" class="ml-2 text-red-600 hover:text-red-900">Delete</button>
                                </td>
                            </tr>
                        </tbody>
                    </table>

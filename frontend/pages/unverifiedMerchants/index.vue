...
[x: string]: any;
    ...
    async function approve() {
        ...
        let isError = false;
        const token = useCookie('token');
        const data = await $fetch<{message:string}>(`${API}/merchant/${selectedId.value}/approve`,{
            method: 'PATCH',
            headers: {
                'Authorization': `Bearer ${token.value}`
            },
    ...
    async function reject() {
        ...
        let isError = false;
        const token = useCookie('token');
        const data = await $fetch<{message:string}>(`${API}/merchant/${selectedId.value}/reject`,{
            method: 'PATCH',
            headers: {
                'Authorization': `Bearer ${token.value}`
            },
        ...
        <div class="flex justify-between">
            ...
            <div class="p-4 w-full h-[100svh] overflow-x-scroll relative">
                ...
                <div class="bg-white rounded-3xl w-full p-4 h-full mt-4">
                    <!-- Approve Modal -->
                    <div v-if="approveModal" class="absolute h-full w-full bg-transparent flex left-0 p-6 justify-center items-center">
                        <div class="w-[500px] bg-white rounded-lg shadow-md p-4 z-4 flex flex-col" @click="">
                            ...
                            <p class="font-bold text-2xl text-center">Approve Merchant</p>
                            <div class="mt-4">
                            ...
                            <div class="flex-1 basis-0 flex justify-center items-end mt-4">  
                                <button @click="approve" value="approve" type="button" class="w-full inline-flex items-center justify-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold capitalize text-white hover:bg-blue-700 active:bg-blue-700 focus:outline-none focus:border-blue-700 focus:ring focus:ring-blue-200 disabled:opacity-25 transition">Confirm</button>
                            </div>
                            <p v-if="isLoading" class="text-center">Loading</p>
                        </div> 
                    </div>
                                        <!-- Reject Modal -->
                                        <div v-if="rejectModal" class="absolute h-full w-full bg-transparent flex left-0 p-6 justify-center items-center">
                        <div class="w-[500px] bg-white rounded-lg shadow-md p-4 z-4 flex flex-col" @click="">
                            <div class="flex justify-end">
                                <button @click="closeModal">
                                    <span class="material-symbols-outlined">close</span>
                            ...
                            <div class="flex-1 basis-0 flex justify-center items-end mt-4">  
                                <button @click="reject" value="reject" type="button" class="w-full inline-flex items-center justify-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold capitalize text-white hover:bg-blue-700 active:bg-blue-700 focus:outline-none focus:border-blue-700 focus:ring focus:ring-blue-200 disabled:opacity-25 transition">Confirm</button>
                            </div>
                            <p v-if="isLoading" class="text-center">Loading</p>
                        </div> 
                    </div>
                    ...
                    <div v-if="deleteModal" class="absolute h-full w-full bg-transparent flex left-0 p-6 justify-center items-center">
                        <div class="w-[500px] bg-white rounded-lg shadow-md p-4 z-4 flex flex-col" @click="">
                            ...
                            <p>This merchant will be archived</p>
                            <div class="mt-4">
                                <label class="block mb-1 font-bold text-lg" for="name">name</label>
                                <p>{{merchants!.filter(merchants => merchants.id == selectedId)[0].name }}</p>
                            </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200" v-for="merchant in merchants" :key="merchant.id">
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap  text-sm font-medium">
                                    <button @click="openModal" value="delete" :id="merchant.id.toString()" type="button" class="text-blue-600 hover:text-red-900 px-4 py-2 border-2 rounded-full">Delete</button>
                                    <button @click="openModal" value="approve" :id="merchant.id.toString()" type="button" class="text-blue-600 hover:text-red-900 px-4 py-2 border-2 rounded-full">Approve</button>
                                    <button @click="openModal" value="reject" :id="merchant.id.toString()" type="button" class="text-red-600 hover:text-blue-900 px-4 py-2 border-2 rounded-full">Reject</button>
                                </td>
                            </tr>
                        </tbody>
                    </table>
